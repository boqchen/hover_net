...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpus', type=str, required=True, help='biomed or dgx:gpu')
    ...
    if args.model_mode == 'fast':
        ...
        patch_margin = 46 # Margin along each edge of the square. Hardcoded in the model architecture.
    else:
    ...
    print(f"Finished creating tissue tiles in {time.time() - tiling_start_time}s")
...
#     os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpus  # Set GPUs 0 and 1 as visible

    # Now, any PyTorch code you run will only use GPUs 0 and 1
    # For example:
...
#     device = torch.device('cuda:' + args.gpus)
    ...
    loader_kwargs = {'num_workers': args.nr_inference_workers,
                     'pin_memory': True}# if "cuda" in device else {}
    print(loader_kwargs)
...
#             print(f'Chunk {chunk_id}/{len(chunks)} -- inferring batch {batch_id}/{len(loader)}...')
        ...
        print(
            f"Finished chunk {chunk_id} in {((time.time() - chunk_time) / 60):.2f} mins ", len(chunks))

    print(
        f"Finished inference on all chunks in {((time.time() - inference_start_time) / 60):.2f} mins")
    print(
    ...
    results = []
    output_file = os.path.join(args.output_dir, slide_id + '_nuclei.csv.gz')
    with gzip.open(output_file, 'wt', newline='') as csvfile:
        writer = csv.DictWriter(
            csvfile,
            fieldnames=['type', 'type_prob', 'centroid_x', 'centroid_y', 'contour']
        )
        writer.writeheader()
        with Pool(processes=args.nr_post_proc_workers) as pool:
            for instances in tqdm.tqdm(
                pool.imap_unordered(postproc_chunk, chunk_feature_files, 1),
                total=len(chunk_feature_files)
            ):
                writer.writerows([
                    {
                        'type': i['type'],
                        ...
                        # Save the nucleus contour using the well-known text (WKT) geometry encoding.
                        'contour': Polygon(i['contour']).wkt
                    } for i in instances
                ])
    ...
    print(
        f"Finished postprocessing of all chunks in {((time.time() - postproc_start_time) / 60):.2f} mins")

    total_runtime = time.time() - pipeline_start_time
    print(f"Total runtime: {((total_runtime / 60) / 60):.2f} hours")
